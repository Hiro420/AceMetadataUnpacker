...
internal sealed class Blowfish
{
	...
	{
		if (key.IsEmpty)
			throw new ArgumentException("Blowfish key must not be empty.", nameof(key));

		InitialP.AsSpan().CopyTo(_p);
		InitialS.AsSpan().CopyTo(_s);
	...
	private uint F(uint x)
	{
		unchecked
		{
			uint h = _s[(x >> 24) & 0xFF] + _s[256 + ((x >> 16) & 0xFF)];
			h ^= _s[512 + ((x >> 8) & 0xFF)];
			h += _s[768 + (x & 0xFF)];
			return h;
		}
